...
\begin{align*}
\mc{A}(x)=-2\left[\mr{Tr}\left\{ \gamma_5 t f(-\Slash{D}^2_x/M^2) \right\} \delta^4(x-y)\right]_{y\to x}
\end{align*}
...
となる．切断関数の変数は
\begin{align*}
-\left[ i\Slash{k} +\frac{\Slash{D}}{M} \right]^2=&-(i\Slash{k})^2-i\Slash{k}\frac{\Slash{D}_x}{M}-\frac{\Slash{D}_x}{M}i\Slash{k}-\left(\frac{\Slash{D}_x}{M}\right)^2 \\
...
&+\frac{1}{2}f''(k^2)\left(-\frac{2ik\cdot D_x}{M}-\left(\frac{\Slash{D}_x}{M}\right)^2\right)^2+\cdots \\
=&\frac{1}{2}f''(k^2)\frac{\Slash{D}^2_x}{M^4} +\cdots
\end{align*}
となるから
\begin{align*}
...
この結果はアノマリーをもつ対称性にともなうカレントを用いて表すことができる．簡単のため，定数の微小パラメータ$\alpha$の対称性変換$\psi(x)\to \psi(x)+it\gamma_5 \alpha \psi(x)$のもとで作用自身が不変だとする．(実際，質量ゼロのフェルミオンによるQEDラグランジアン(8.6.1)を考えると
\begin{align*}
\mc{L}(x)=-\frac{1}{4}F^{\mu\nu}F_{\mu\nu} -\bar{\psi}\left[\gamma^\mu (\partial_\mu+ieA_\mu)\right]\psi
\end{align*}
となっており，これはこの変換のもとで不変になっている．)その場合，(7.3.14)で示したように時空に\uwave{依存する}パラメータ$\alpha(x)$で変換を行うと，作用には$\delta I=\int d^4x J^\mu_5 (x)\partial_\mu \alpha(x)$だけの変化が生じる．\footnote{この項は作用の$\int d^4x J^\mu_\alpha (x)A_{\alpha\mu}(x)$から$\delta A_{\alpha\mu}=\partial_\mu \epsilon_\alpha+\cdots $で生じるものとは別である．実際，上のmasslessフェルミオンのQEDラグランジアンから(7.3.15)を用いて計算すると$J_5^\mu=-ie\bar{\psi}\gamma^\mu \gamma_5 \psi$となり，電流$J^\mu =-ie\bar{\psi}\gamma^\mu \psi$とは別物である．masslessなゲージ場$A^\mu_\alpha$と結合するゲージカレント$J^\mu_\alpha$はゲージ対称性であるから，アノマリーによって破れるわけにはいかない．でなければ5.9節で説明したようにローレンツ不変性を保つためにmasslessゲージ場と結合するカレントは保存しなければならないから，厳密になりたつべき対称性であるローレンツ不変性が破れてしまう．}．ここで$J^\mu_5(x)$は，場についての任意の変化のもとで作用が停留するように場の演算子が動的な方程式を満たす時には保存されるカレントだ．すなわち，古典的作用の変化が停留する$\delta I=0$と仮定すると部分積分よりカレントは
\begin{align*}
...
\end{align*}
さて，この生成子$\gamma_5 \tau_3$による対称性(22.2.33)$U(1)_A \subset SU(2)_A$が完全に破れる低エネルギーでの，パイオンの有効相互作用を考える(19.7節参照)．変換(22.2.33)の対称性のもとで$\tau_3$に結合するNGボゾンである$\pi^0$場は
\begin{align*}
...
\end{align*}
この有効ラグランジアンは実際，変換(22.2.36)のもとで(22.2.35)を与える．これを$\pi^0\to2\gamma$崩壊の有効ラグランジアンの一般的な表式(22.1.1)と比較すると，(22.1.1)の定数$g$は
\begin{align*}
...
\end{align*}
ここで，$\mc{C}$は(5.4.35)(5.4.36)
\begin{align*}
\mc{C}\gamma^T_\mu \mc{C}^{-1}=-\gamma_\mu  ,\quad \mc{C}=\gamma_2\gamma_4 
\end{align*}
で定義された行列だ．(すなわち，下成分は$(1/2,0)$表現に属する右手フェルミオンを$(1/2,0)$表現に変換したものを表しており，これで全ての左手フェルミオン(もともと左手場のものはもちろん，右手場もこの変換により一つの左手場)を一つのフェルミオン場として統一して記すことができている．)フェルミオン数(バリオン数か，バリオン数引くレプトン数)を保存する微小ゲージ変換
\begin{align*}
...
\end{align*}
だ．ここで$j^\mu_\alpha$はフェルミオン的カレントで自由場$\chi$を使って
\begin{align*}
