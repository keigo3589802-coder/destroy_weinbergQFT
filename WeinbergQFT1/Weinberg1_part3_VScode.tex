...
注意．$S_{\beta\alpha}=\delta(\beta-\alpha)-2\pi iM_{\beta\alpha}\delta^4(p_\beta-p_\alpha)$の$\delta(\beta-\alpha)$部分は$S$演算子のダイソン級数の最低次の単位演算子$1$から生じるものだと思うかもしれない．しかし実際には相互作用の各次数から$\delta(\beta-\alpha)$の因子に比例するものとそうでないものが現れる．すなわち
\begin{align*}
S_{\beta\alpha}=&\delta(\beta-\alpha)+\Bigl(\delta(\beta-\alpha)-2\pi i \delta^4(p_\beta -p_\alpha)M^{(1)}_{\beta\alpha}\Bigr)+\Bigl(\delta(\beta-\alpha)-2\pi i \delta^4(p_\beta -p_\alpha)M^{(2)}_{\beta\alpha}\Bigr) +\cdots \\
=&\delta(\beta-\alpha)-2\pi i \delta^4(p_\beta -p_\alpha )\sum_{n}M^{(n)}_{\beta\alpha}
\end{align*}
という構造になっている．ここで$M^{(n)}_{\beta\alpha}$は，ダイソン級数の相互作用について$n$次の項から現れる自明でない寄与である．詳しくは4.4節で扱う．
...
\end{align*}
特に，これを1粒子に適用すると，任意の粒子の崩壊率$\Gamma_\alpha$はスピンを反転させた反粒子の崩壊率に等しいことがわかる．1粒子の場合のスピンの向きは回転により自由に変更でき，回転不変性により崩壊率はスピンの$z$成分によらないはずである．ｓたがって一般的な結果(3.6.15)の特殊な場合として，不安定粒子とその対応する反粒子とは性格に同じ寿命を持つことが分かる．(もちろん，各崩壊チャンネルごとの崩壊率は同じになるとは限らない．)
